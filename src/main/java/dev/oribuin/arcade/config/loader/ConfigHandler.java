package dev.oribuin.arcade.config.loader;

import dev.oribuin.arcade.ArcadePlugin;
import dev.oribuin.arcade.config.serializer.AttributeSerializer;
import dev.oribuin.arcade.config.serializer.ComponentSerializer;
import dev.oribuin.arcade.config.serializer.DurationSerializer;
import dev.oribuin.arcade.config.serializer.EnchantSerializer;
import dev.oribuin.arcade.config.serializer.SoundSerializer;
...
public class ConfigHandler<T> {
    ...
    public ConfigHandler(Class<T> configClass, String name) {
        this(configClass, Paths.get("plugins", ArcadePlugin.getInstance().getName()), name);
    }
    ...
    public void load() {
        try {
            ...
            ArcadePlugin.getInstance().getLogger().severe("Failed to load configuration [" + this.configFile.getPath() + "] due to: " + ex.getMessage());
        }
    }
    ...
    public void save() {
        try {
            ...
            ArcadePlugin.getInstance().getLogger().severe("Failed to save configuration [" + this.configFile.getPath() + "] due to: " + ex.getMessage());
        }
    }
